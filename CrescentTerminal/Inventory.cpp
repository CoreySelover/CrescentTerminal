...
std::shared_ptr<Item> Inventory::removeItemAt(sf::Vector2i position)
{
	if (position.x >= int(m_items.size()) || position.y >= int(m_items[0].size()) || position.x < 0 || position.y < 0)
	{
		throw std::invalid_argument("Invalid position");
	}
	auto item = m_items[position.x][position.y];
	if (!item) { throw std::invalid_argument("No item at position"); }
	m_items[position.x][position.y] = nullptr;
	return item;
}

std::shared_ptr<Item> Inventory::getItemAt(sf::Vector2i position) const
{
	if (position.x >= int(m_items.size()) || position.y >= int(m_items[0].size()) || position.x < 0 || position.y < 0)
	{
		throw std::invalid_argument("Invalid position");
